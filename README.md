...
21. 对显式 JSON/Markdown artifact 执行跨来源 assertion 与十案例 gold evaluation，验证冲突拒答、line-range citation 和三次运行一致性。
...
- `run-review-eval` 使用仓库内 gold locator 和结构化期望，不使用 LLM judge；当前十案例是回归基线，不代表量产评审准确率。
