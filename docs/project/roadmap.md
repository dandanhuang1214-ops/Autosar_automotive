...
- R5q 本地实现与验证已完成（2026-09-07）：新增跨平台可控 SHA-256 mismatch 演练，CI 直接观察 CLI failure 后校验 rejection artifact、无 request/result 物化，并独立上传拒绝证据；远端验收已修复诊断 extra、matrix fail-fast、CRLF hash 漂移与 blocked reason 平台断言，Windows/Ubuntu 最终重跑待确认。
