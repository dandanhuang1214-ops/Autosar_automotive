...
| P18 项目回归比较（本地已验收） | baseline/candidate 项目报告 → 阶段、验收项和 finding 差异 | 稳定/两类回归/反向改善、双侧精确证据、不可比较与篡改拒绝；远端待验收 |
