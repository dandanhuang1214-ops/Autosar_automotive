...
| P17 项目级审查（本地已验收） | 用实际失败报告回答工程问题并引用证据 | 正常/两类失败重放、精确引用、缺文件及证据不足拒答；远端待验收 |
...
P15/P16 已由修复提交 `af699c1` 的远端 run `34734838719` 完成七 job 跨平台验收。P17 已完成本地实现与公开失败集验收，当前等待远端七 job 跨平台复验。P16/P17 的输入仍是公开合成 DOCX 与 virtual CAN；不外推为客户数据、物理 ECU 或量产交付。详见 [P16 记录](../research/p16-generate-arxml-artifact-bridge-2026-09-12.md)。
