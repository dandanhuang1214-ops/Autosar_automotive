...
日期：2026-09-24。范围：声明驱动的多项目通信验证，包含 P20a–d。当前整阶段状态为 local-accepted（本地最终回归通过，远端待验收）。此前 P20a/P20b 的 run 不代替本次项目集成实现的 CI。
...
| Windows/Ubuntu/Python 3.14 | 本次实现提交的七 job 与 runtime-evidence artifact | 待远端验收 |
