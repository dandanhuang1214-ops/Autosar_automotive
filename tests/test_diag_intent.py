...
class UdsIntentTests(unittest.TestCase):
    def test_loads_public_window_control_diagnostic_intent(self) -> None:
        ...
        self.assertEqual(summary["scenarios"][8]["reset_type"], "hardReset")
        self.assertEqual(summary["scenarios"][15]["group_hex"], "0xFFFFFF")
    ...
    def test_rejects_clear_for_unknown_dtc_group(self) -> None:
        payload = json.loads(INTENT.read_text(encoding="utf-8"))
        payload["scenarios"][15]["group"] = 0x123456
    ...
    def test_rejects_unsupported_reset_type(self) -> None:
        payload = json.loads(INTENT.read_text(encoding="utf-8"))
        payload["scenarios"][8]["reset_type"] = "softReset"

        with tempfile.TemporaryDirectory() as directory:
            path = _write_intent(directory, payload)
            with self.assertRaisesRegex(ValueError, "only hardReset"):
                load_uds_intent(path)
